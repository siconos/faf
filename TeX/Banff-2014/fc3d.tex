...
\frame{
  ...
  \begin{block}{Properties}
    ...
    \item nonmonotone since the mapping $F$ is no monotone for large $\mu$
    \item many possible reformulations such as Nonsmooth equations $G(r) = 0$ \\
      natural map $F^\nat$ %associated with the VI (\ref{eq:inclusion-1}) $ F^\nat(z) = z - P_{X}(z-F(z))$.\\
      and variants of this map (Alart-Curnier formulation, \ldots)
    \end{itemize}
  \end{block}
}  
\frame{
  ...
  \begin{block}{Important Remarks}
    ...
    \item The problem is very hard to solve efficiently.
...
\frame{
  ...
  \begin{block}{Semismooth Newton methods for nonsmooth equations $G(r) = 0$.}
    ...
    \item Pioneering work of Alart \& Curnier
      \begin{equation*}
        \label{eq:CKPS-1}
        \begin{cases}
          r_\n - P_{\nbR^{n_c}_+}(r_\n - \rho_\n  u_\n) = 0, \\
    ...
    \item other SOCCP function (Fisher-Bursmeister function)
