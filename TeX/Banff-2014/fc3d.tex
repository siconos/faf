...
\frame{
  ...
  \begin{block}{Second Order Cone Complementarity (SOCCP) formulation}
    ...
    \begin{itemize}
    \item Modified relative velocity $\hat u \in \nbR^3$ defined by \citep{DeSaxce92}
      \begin{equation}
...
\frame{
  ...
  \only<1>{ \begin{block}{Wide range of applications}
      ...
      \item is really representative in the linear or linearized case (Newton procedure),
  ...
  \begin{block}{Origin of the linear relation $u = H^\top v + w$}
     ...
     \item $w$ can contain
       \begin{itemize}
       \item impact laws terms or prescribed velocity in velocity level formulation
       \item displacements or increments of displacements in position level formulation
       \end{itemize}
     \end{itemize}
   \end{block}
   }
...
\frame{
  ...
  \begin{block}{Complementarity problem / Variational inequality}
    Discrete frictional contact are\\[1mm]
    ...
    \end{itemize}
  \end{block}
  \ding{220} the variational inequality is NOT  the optimality condition of a (convex) optimization problem.\\[1mm]
...
\frame{
  \frametitle{From the optimization point of view} 
    ...
    % \end{itemize}
    % 
   \begin{block}{Many possible reformulations}
     \begin{itemize}
   ...
   \begin{block}{Numerical methods}
     \begin{itemize}
     \item Numerous numerical methods exist. 
     \item Many of these are adaptations of mathematical programming methods.
     \end{itemize}
   \end{block}
 }
 \frame{
  ...
  \begin{block}{An optimization problem}
    ...
    \begin{equation}
      \label{eq:2}
      \begin{array}{lcl}
        \min_{v,u,r} & & \hat u ^\top r = u^\top r + \mu r_\n \|u_{\t}\|  \stackrel{\Delta}= b(u,r)\\[1mm]
        \text{s.t.} & & Mv = H r + f \\[1mm]
                    & & \hat u = H^\top v + w + g(u) \in K^\star \\[1mm]
                    & & r \in K 
