...
\section{Existing numerical solvers}
\label{Sec:Numerics}

\frame{
  \frametitle{Existing numerical solution procedures}
  \begin{itemize}
...
\frame{
  ...
  \begin{block}{Variational Inequality (VI) reformulation}
  \begin{equation}
    \label{eq:1}
   \eqref{eq:soccp2} \Longleftrightarrow  - F(r) := -( W r +q +g(Wr+q))  \in N_K(r)
  \end{equation}
\end{block}
\begin{block}{Standard methods}
    \begin{itemize}
    \item Basic fixed point iterations with projection\hfill\tr{\sf{[FP-VI]}}
      $$\sf r_{k+1} \leftarrow P_{K}(r_k - \rho_k\,F(r_k))$$
    \item  with fixed $\rho_k = \rho$, we get the Uzawa Algorithm of \cite{DeSaxce.Feng_MCM1998} with similarity with augmented Lagrangian methods\citep{Wriggers2006}\hfill\tr{\sf{[FP-DS]}}
    \item Extragradient method\hfill\tr{\sf{[EG-VI]}}
    $$\sf r_{k+1} \leftarrow P_K(r_{k}-\rho_k\,F(P_K(r_k-\rho_k F(r_k))))$$
  \end{itemize}
\end{block}
...
{Self-adaptive procedure for $\rho_k$\hfill\tr{\sf{[UPK]}}}
 \begin{equation*}
   ...
   \left\{  \begin{array}{l}
              \rho_k = \rho 2^{m_k},\\[1mm]
              \rho_k \| F(r_k)-F(\bar r_k) \| \leq \|r_k-\bar r_k\|
            \end{array}\right.
 \end{equation*}
\end{block}
    ...
    \begin{equation}\label{EQ:NSGS-local1}
      \left\{ 
        \begin{array}{l}
          ...
          ~\\
          \widetilde u^{\alpha}_{i+1} = \left[u_{\n,i+1}^{\alpha}+ \mu^{\alpha}\;||u^{\alpha}_{\t,i+1}||, u^{\alpha}_{\t,i+1}\right]^T \\ \\
          {\bf K}^{\alpha,*} \ni \widetilde u^{\alpha}_{i+1}  \perp r^{\alpha}_{i+1} \in {\bf K}^{\alpha} \\
        \end{array} \right.
    \end{equation}
...
\begin{frame}
  ...
  Optimization, contact and huge-scale problems. \citep{dostal2016scalable}
  
  \includegraphics[height=0.4\textheight]{Dostal_book_2023.pdf}
\end{frame}
