...
\title[Coulomb friction and optimisation ]{ {\normalsize Laboratoire de Mécanique de Paris Saclay. Seminar LMPS}\\[2mm] Non-smooth dynamical systems.\\
  Solving the contact problem with friction using methods derived from large-scale optimisation\\[-2mm]}

 \author{Vincent Acary\\[1mm] Inria -  Centre de l'Université Grenoble Alpes - Laboratoire Jean Kuntzmann}
\date{\today}
 \institute{ I
   \includegraphics[height=0.16\textheight]{./logos/inr_logo_rouge.jpg}\hfill
   \includegraphics[height=0.16\textheight]{./logos/logo_ljk2.pdf}\hfill
   \includegraphics[height=0.16\textheight]{./logos/logo_uga_transparent.png}}
...
\frame{\titlepage
    ...
    Claude L\'emar\'echal, J\'er\^ome Malick,\\ Hoang Minh Nguyen and Mathieu Renouf 
  \end{center}
...
%   \end{itemize}
% \end{frame}
\includepdf[pages={2-5}]{TRIPOP_short.pdf}
\includepdf[pages={1-4}]{IllustrationsApplications.pdf}

\begin{frame}
  \frametitle{Contents}
  \begin{center}
   \tr{\Large  Solving the contact problem with friction using methods derived from large-scale optimisation. }
  \end{center}
  \begin{enumerate}
  ...
  \item { } [\textcolor{blue}{\sc SKIPPED}] Provide an existence result, whose assumption can be verified numerically .
...
\frame{
  ...
  \begin{block}{Conclusions}
    \begin{itemize}
    \item Further research is needed to develop a robust AND efficient solver.
    \item The IPM and ANM numerical methods provide robust solvers.
    \item Coupling with other physical phenomena to obtain a monolithic variational inequality : \\[1mm]
      ...
      $\bullet$ and other funny mechanics...
    \end{itemize}
  \end{block}
