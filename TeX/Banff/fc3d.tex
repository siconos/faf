...
\frame{
  ...
  \begin{block}{SOCCP formulation}
    ...
    \item  Second-Order Cone Complementarity Problem (SOCCP) 
      ...
      \end{equation}
      \item 
      References : \cite{Acary.Brogliato2008,ZAMM:ZAMM201000073,DeSaxce92}.
...
\frame{
  ...
  \begin{problem}[Reduced discrete frictional contact problem]\label{prob:II}
    ...
    \qed
  \end{problem}
  with $ W = H^\top M^{-1} H$ and $q = H^\top  M^{-1} f + w $.
}
\frame{
  \frametitle{3D frictional contact problems} 
  This problem is really representative of a lot of applications.

  Origin of the linear relations~\cite{Acary.Cadoux2013}.
  ...
  \item Time--discretization  of the discrete dynamical mechanical system
    ...
    \end{itemize}
  \item   Time--discretization  and space discretization of the elasto dynamic problem of solids
...
\frame{
  ...
  \begin{block}{Nonmonotone and nonsmooth problem}
    ...
    \item if we neglect $g(\cdot)$, (\ref{eq:soccp3}) is a gentle
      monotone SOCLCP that can be associated to a convex SOCQP.
    \item otherwise, the problem is nonmontone and nonsmooth since
      $g()$ is nonsmooth
    \end{itemize}
   ...
   \begin{block}{Possible reformulation}
     ...
     \item Nonsmooth equations $G(r) = 0$\\
        ...
        \begin{equation}
          ...
          F^\nat(z) = z - P_{X}(z-F(z)),
        \end{equation}
        and its variants (Alart-Curnier formulation)\\
        SOCCP-functions. (Fisher-Bursmeister function)
     \end{itemize}
   \end{block}
 }
