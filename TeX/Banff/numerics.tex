...
\frame{
  \frametitle{Proximal point algorithms \cite{Moreau1967,Rockafellar1976}}
  \begin{block}{Problem to solve}
    \begin{equation}
      \label{eq:min}
      \min_{x} f(x)
    \end{equation} with as a special case $$ f(x) = \frac 1 2 G^\top(x) G(x)$$
    We define the approximation problem for a given $x_k$
    \begin{equation}
      \label{eq:min}
      x^\star = \argmin_{x}\, f(x) + \rho \|x-x_k\|^2 \stackrel{\Delta}{=} \prox_{f,rho} (x_k)
    \end{equation}
  \end{block}
