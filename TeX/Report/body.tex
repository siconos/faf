...
In Figure~\ref{fig:VI/UpdateRule}, we compare the different numerical solvers for VI described in Section~\ref{sec:numericalmethods,vi}. Only the test sets for which the solvers are reached the precicion before the prescribed time (timeout) are presented. For that reason, the results for the test sets LowWall\_FEM,  Cubes\_H8, Bridge\_PR, AqueducPR, 945\_SP\_Box\_PL, 100\_PR\_PerioBox, BoxesStack, Chute\_4000  and Chute\_1000  are not depicted. The solvers are quite slow to converge in practice for large problems although they are robust except for {\sf FP-DS}. 
...
Again the results on the test sets Bridge\_PR, 100\_PR\_PerioBox, Chute\_local\_problems, BoxesStack are not reported since the  {\sf NSGS-$\star$} are not able to meet the required tolerance for a sufficient percentage of the problems mainly due  to a slow convergence rate. We can see that this problem also appears in the Capsules test set.
