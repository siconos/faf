...
\end{figure}
In figure~\ref{fig:profile-Capsules-NSGS-1_3--tol-local-time}, the tolerance of the local solver is varied and its effect on the global convergence of the solver is reported. For the ``Capsules'' set of examples, an increase of the tolerance of the local solver yields a increase of the computational effort. In fact the number of iteration of the NSGS algorithm does not change but the local solver is more demanding. There is no interest to ask for a higher accuracy of the local solver. Surprisingly, the algorithm is also able to reach the global accuracy of $tol = 10^{-8}$ with a suite low accuracy of the local solver. This mainly due to the fact that at least one iteration of the local solver is always done and the set of examples are not so difficult to solve. {\tt XXX can we give an numerical argument for this claim ?}

Let us have a look to more difficult examples in Figure~\ref{fig:profile-LMGC_Bridge_PR-NSGS-1_3--tol-local-time}.
\begin{figure}
  \centering
