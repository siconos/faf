...
\end{equation}
A well-known result (see~\cite{Facchinei.Pang2003}) states that the solutions of a VI are \tg{related} to the zeroes of the natural map and normal map:
\begin{equation}
...
In the seminal work of~\cite{Sibony1970}, iterative methods for solving monotone VIs are based on the natural map and fixed point iterations. The role of $\rho$ is recognized to be very important for the rate of convergence. To improve the methods,~\citet{Sibony1970} proposes to use ``skewed'' projector based on a non-Euclidean metric. Given a positive definite matrix $D\in R^{n\times n}$, a skewed projector $P_{X,D}$ onto $X$ \tg{is defined as follows:} $z  = P_{X,D}(x)$ is the unique solution of the convex programm
\begin{equation}
  \label{eq:opt-proj-skew}
  \begin{cases}
    \min\, \Frac 1 2 (y-x)^\top D (y-x), \\[2mm]
    \begin{array}{ll}
    s.t. & y \in X .
  \end{array}
  \end{cases}
...
\begin{ndroh}
 Est ce qu'on veut dire plus de chose avec la phrase précedente. Je trouve que actuellement, cela tombe un peu à plat.
Si on souhaite juste mentionner ce papier, je propose qu'on remonte la citation pour la mettre au même niveau que celle de M\&J.
\end{ndroh}
...
- The notion of C-function is used, but is not defined.
\end{ndroh}
...
\begin{ndrva}
  ...
  \end{equation}
  Check there is no trouble with $\mu =0$. This has to be done for all of this section.
\end{ndrva}
...
\paragraph{The alternating optimization \tg{approach}} aims at solving the frictional contact problem by alternatively solving the Signorini condition for a fixed value of the tangential reaction $r_\t$, and solving the Coulomb friction model for a fixed value of the normal reaction $r_\n$. 
...
\end{equation}
yielding to Algorithm~\ref{Algo:FP-vi} with the specific choice of $D=\rho_k^{-1} \, I$. The choice of the updating rule of $\rho_k$ is detailed in Section~\ref{sec:numericalmethods,vi,adaptive}.
\begin{algorithm}
  \begin{algorithmic}
    {\sf
      \STATE $ $
...
\begin{equation}
  ...
  %\end{cases}
\end{equation}
In the sequel when a parameter $\rho$ is specified, it is assumed that \tg{$D = \rho^{-1}I$}.
...
\begin{ndroh}
 2, 3 sentences motivating this subsections would be a nice addition. Will try to do it.
\end{ndroh}
...
Splitting techniques are standard techniques to solve $\mathrm{VI}(F,X)$ when the function $F$ is affine, that is $F(z) = Mz+q$.
Usually, a block splitting of the matrix $M$ is performed and a Projected Successive Over Relaxation (PSOR) \tg{method} is used to solve the VI.
\tg{Since the cone $K$ is a product of second-order cones in $\RR^3$, a natural way to split the problem is to form sub-problems by using single contact as a building block.}
The sub-problems can be solved by any method for the VI that have been presented in the previous sections. In the same way, the proximal point algorithm can also be used which amounts to solving the original $\mathrm{VI}(F,X)$ by solving a sequence of $\mathrm{VI}(F_{c,x_k},X)$  problems such that $  F_{c,x_k}(z) = z - x_k + c F(z) , \quad c > 0$ and $\lim_ {k \rightarrow +\infty } \|x_k-z\| =0$.
 ...
 \begin{ndroh}
  I'm not sure what to do with this subsection. It is quite short and mention a lot of notions that have not been introduced yet. Should we keep it here or move it to a later Section.
 \end{ndroh}
...
\cite{Haslinger.ea_CMAME2002} \&~\cite{Dostal_JCAM2002}: successive approximation in 2D a) classical Tresca iteration (FPMI).  b) splitting N/T plus a Fixed point (FPMII). Use of a specific QP solver for box constraint~\cite{Dostal_SIOPT1997} that is an improvement of MOre Toraldo method.
