...
In Algorithms~\ref{Algo:PPA-vi} and~\ref{Algo:PSOR-II}, an internal tolerance is used to control the accuracy of the internal solver. It is generally not useful to solve the internal problem at the accuracy of the global one. In the comparison study, we set the internal tolerance $tol_{{int}}$ to a fraction of the error $error/10.0$ for the Algorithm~\ref{Algo:PPA-vi}. For Algorithm~\ref{Algo:PSOR-II}, the local tolerance  $tol_{{local}}$ is  set by default to a very low value of $10^{-14}$. When it is specified, an adaptive local tolerance is set to $error/10.0$.
...
\paragraph{Effect and influence of the local solver in {\sf NSGS-$\star$}  algorithms}
