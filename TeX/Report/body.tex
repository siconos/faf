...
\moh{In this section, we formulate an abstract, algebraic finite--dimensional frictional contact problem. We cast this problem as a complementarity problem over cones, and discuss the properties of the latter.
...
\moh{The problem data are: a symmetric positive (semi-) definite matrix ${M} \in \RR^{n \times n}$, a vector $ {f} \in \RR^n$, a matrix  ${H} \in \RR^{n \times m}$ with $m= 3n_c$, a vector $w \in \RR^{m}$ and a vector of coefficients of friction $\mu \in \RR^{n_c}$.}
...
\item Mylapilli.Jain\_JCND2016.pdf 
\item \cite{Heyn.ea_IJNME2013}
\item \cite{Krabbenhoft.ea_CMAME2012}
\end{enumerate}
...
\end{equation}
%
The contact force exerted by $A$ on $B$ is denoted by $r_G \in \RR^3$ and is decomposed as
\begin{equation}
  \label{eq:reaction}
  r_G = r_{\n} {\sf N} +   r_{\tone}{\sf T_1} + r_{\ttwo}{\sf T_2}  ,\quad \text{ with  } r_{\n} \in \RR\;\text{and}\; r_{\t}\coloneqq[r_{\tone},r_{\ttwo}]^\top \in \RR^2.
\end{equation}
In the local frame, the reaction force is given by
\begin{equation}
 r\coloneqq [r_{\n}, r_{\t}] \in \RR^3.
\end{equation}
%
...
\tg{To this end}, the relative velocity $u_G \in \RR^3$ of the point $C_{B}$ with respect to $C_{A}$ is also decomposed as
\begin{equation}
  u_G =  u_\n {\sf N} +  u_{\tone}{\sf T_1} + u_{\ttwo}{\sf T_2}  \quad \text{ with } u_\n \in \RR \text{ and } u_\t=[u_{\tone},u_{\ttwo}]^\top  \in \RR^2.
\end{equation}
In the local frame, the relative velocity is given by
\begin{equation}
 u\coloneqq [u_{\n}, u_{\t}] \in \RR^3.
\end{equation}
%
...
Coulomb's friction  models the frictional behavior of the contact force law in the tangent plane \tb{ spanned by $(T_1,T_2)$}. Let us define the Coulomb friction  cone $K_\mu$ is the second order cone (Lorentz or ice--cream cone), where $\mu$ is the coefficient of friction. The Coulomb friction states for the sticking case that
\begin{equation}
...
\end{equation}
if $g_{\n} \leq 0$ and $r_\n =0 $ otherwise. An inclusion form of the Coulomb friction \moh{for the tangential part} can be also proposed: let $D(\cdot)$ be the Coulomb disk:
\begin{equation}
  \label{eq:diskR}
...
\begin{equation}
  ...
  - u_{\t} \in N_{D(\mu r_\n)}(r_{\t}).
\end{equation}
Since $D(\mu r_\n)$ is not a cone, the inclusion~\eqref{eq:Coulomb-inclusion} is not a complementarity problem, \moh{but a variational inequality}.
...
\end{align}
%
\moh{We are now ready to define the mathematical problem that we solve.}
%\settheoremtag{FC}
...
\moh{Therefore, it makes sense to design a procedure to solve the problem}. In the sequel, we will compare numerical methods only when this assumption is satisfied.
...
\tb{The assumption is also satisfied \moh{whenever} $\mathrm{im}\, H = \RR^m$ or in other words if $H^\top$ has full row rank. Unfortunately, in large number of applications $H^\top$ is rank deficient. }
...
\tg{If} the matrix $H$ has full rank and that friction coefficients are ``small'', a classical argument for the uniqueness of solution of VIs can be satisfied.
\tg{Note that the full rank hypothesis on $H$ implies that $W$ is positive-definite. Therefore,} we have  $(x-y)^\top W (x-y) \geq C_W \|x-y\|^2$ with  $C_W>0$.
...
\begin{equation}
  ...
  \end{array}
\end{equation}
\tg{Note that} for small values of the coefficients of friction \moh{the first term in the right-hand side dominates the second one}.
...
In this section, \moh{we expose a classical approach to solving a VI or a QVI, based on a reformulation of the inclusion as an nonsmooth equation.}
\moh{The term nonsmooth equation highlights that the mapping we consider fails to be differentiable.
...
\begin{equation}
  ...
  G(r) = 0
\end{equation}
where $G$ is generally \tg{only} locally Lipschitz continuous. \moh{The mapping $G$ is such that} the zeroes $(r)$ of~\eqref{eq:ne-1} are the solutions of~\eqref{eq:soccp2}.
...
\moh{A general-purpose reformulation of VI is obtained by using} the normal and natural maps, see~\cite{Facchinei.Pang2003} for details.
...
\end{equation}
where $P_X$ is the Euclidean projector on the set $X$. A well-known result (see~\cite{Facchinei.Pang2003}) states that the solutions of a VI are \tg{related} to the zeroes of the natural map :
\begin{equation}
...
\begin{equation}
  ...
  \end{array}
\end{equation}
The \tg{relations given in}~\eqref{eq:VI-normalnatural} continue to hold for the parametric mapping.
...
To this aim, \tg{we augment the problem by introduction a auxiliary variable $s$, the image of $g(u)$ introduced in~\eqref{eq:modified}.
We then obtain}
