...
In Figure~\ref{fig:VI/UpdateRule}, we compare the different numerical solvers for VI described in Section~\ref{sec:numericalmethods,vi}. Only the test sets for which the solvers are reached the precicion before the prescribed time (timeout) are presented. For that reason, the results for the test sets LowWall\_FEM,  Cubes\_H8, Bridge\_PR, AqueducPR, 945\_SP\_Box\_PL, 100\_PR\_PerioBox, Chute\_4000  and Chute\_1000  are not depicted. The solvers are quite slow to converge in practice for large problems although they are robust except for {\sf FP-DS}. 
...
\item On the test sets 100\_PR\_PerioBox II, KaplasTower,  Chain, Capsules, the solver {\sf NSGS-NSN-$\star$} are the best solvers and behave very well on Bridge\_PR II. It seems that when a tight accuracy is required, the solvers {\sf NSGS-NSN-$\star$} are useful and helps with a tight local tolerance to speed-up the convergence.
...
\begin{enumerate}
\item For the test sets that are quickly solved (see Table~\ref{Tab:fclib-simulation}), such as BoxesStack,  Capsules, Chain, Chute\_local\_problems, KaplasTower, KaplasTower II,  a tight tolerance on the local solver $10^{-16}$ improves the efficiency  of the {\sf NSGS-FP-VI-UPK} solver. 
...
In Figure~\ref{fig:PSOR}, we study the effect  on the performance of the relaxation parameter $\omega$ ranging from $[0.5,1.8]$.  Two conclusions can be drawn:
\begin{enumerate}
...
\item For the rigid tests, the effect of the relaxation is not so clear. For values of $\omega$ greater than $1.0$, the efficiency is improved but the robustness deteriorates and we observe the contrary for the  $\omega$  less than $1.0$
\end{enumerate}
