...
In \citep{Hueber.ea_SJSC2008}, and subsequently in~\citep{Koziara.Bicanic_CMAME2008}, another function is used to reformulate the problem~\ref{prob:II}:
\begin{equation}
  \label{eq:HSW-II}
...
When the $G$ is nonsmooth but Lipschitz continuous, as it is the case in the equation reformulations of VIs, the Jacobian matrix is replaced by an element of the subdifferential at $z$ denoted by $ \Phi(z) \in \partial G(z)$.
...
Recall general results of Qi Sun.


\paragraph{Estimation of $\rho, \rho_\n, \rho_t$ parameters}
...
\paragraph{Adaptive estimation of the parameters}

see \cite{Koziara.Bicanic_CMAME2008}.
...
\subsection{Robustness issues in semismooth Newton methods.}
\begin{ndrva}
\begin{itemize}
\item semi--smoothness ok.
\item semismooth locally convergent provided the all the generalized gradient are regular at solution. degeneracy in $W$
\item far from the solution the iteration matrix may ill conditionned or irregular.
\item choice of good rho parameters helps a lot but not sufficient.
\item work of Hueber et al. in that direction
\item analsis of Alart and Jourdan Alrt with respect to the rho parameters and the coefficient of friction
\end{itemize}
\end{ndrva}
...
Goldstein-Price line-search is also discussed and it is advised to choose
...
\begin{algorithm}
  \begin{algorithmic}
    {\sf
      ...
      \REQUIRE $t \to q(t)$, for $t \geq 0$, with $q'(0) < 0$ a merit function representing $f(x + t d)$
...
\begin{algorithm}
  \begin{algorithmic}
    {\sf
      ...
      \REQUIRE $t$, an initial stepsize-value.
      \REQUIRE $t \to q(t)$, for $t \geq 0$, with $q'(0) < 0$ a merit function representing $f(x + t d)$
      \REQUIRE $m$, a parameter with $0 < m < 1$
      \ENSURE a finite line-search
      \WHILE {$m q'(0) < \frac{q(t)-q(0)}{t}$}
      \IF{$t_R = 0$}
      \STATE{$t \leftarrow a t$}
      \ELSE
      \STATE{$t_R \leftarrow t$}
...
Book of Dostal  ~\cite{Dostal.ea_AMM2016}
\end{ndrva}
...
\begin{algorithm}
  \begin{algorithmic}
    {\sf
      ...
      \STATE solve~(\ref{eq:ACLM-4}) for $r_{k+1}$
...
\subsection{Control of the tolerance of internal solvers in optimization approach}
