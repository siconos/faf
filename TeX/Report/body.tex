...
\subsubsection{Alart--Curnier's  function}
...
The development of such formulation of discrete contact problem with friction dates bake to the seminal work of  Alart \& Curnier~\cite{Curnier.Alart88,Alart.Curnier1991}. This nonsmooth equation reformulation was motivated by the development of nonsmooth (or generalized) Newton methods (see Sect.~\ref{Sec:NSN-AC} ). To more accurate, the original Alart--Curnier function is given by 
\begin{equation}
  \label{eq:AC-1}
  \begin{cases}
    r_\n - P_{\RR^{n_c}_+}(r_\n - \rho_\n  u_\n) = 0, \\
  ...
  \end{cases}
\end{equation}
Note that the difference between~(\ref{eq:CKPS-1}) and (\ref{eq:AC-1}) lies in the formulation disk onto which one project: $D(\mu, (r_\n - \rho u_\n)_+)$ rather than $D(\mu, (r_{\n})_+)$. 
...
In~\cite{Christensen.Klarbring.ea1998}, 
