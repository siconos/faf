...
\def\subfiglayout{\captionsetup[subfloat]{farskip=-2pt,captionskip=-5pt,font=scriptsize}}
...
\paragraph{Evaluation of the influence of the self--adaptive procedure for step length} In Figure~\ref{fig:VI/UpdateRule}, we compare the different numerical solver for VI. Only the test sets for which the solvers are reached the precicion before timeout are presented. For that reason, the results for the test sets LowWall\_FEM,  Cubes\_H8, Bridge\_PR, AqueducPR, 945\_SP\_Box\_PL, 100\_PR\_PerioBox, Chute\_4000  and Chute\_1000  are not presented. The solvers are quite low in practice for a large number of contacts and they failed to meet the required tolerance in time for the examples that are not depicted in Figure~\ref{fig:VI/UpdateRule}. The main conclusions are as follows:
\begin{enumerate}
...
\begin{figure*}[htbp]
  \centering
  \subfiglayout
\commentedfigure{\subfloat[\scriptsize LowWall\_FEM]
...
%    {\includegraphics[width=\figwidth]{../figure/VI/UpdateRule/1.0e-08/100/time/profile-LMGC_100_PR_PerioBox.pdf}} \\
...
\subfloat[\scriptsize BoxesStack1]
...
Again the results on the test sets Bridge\_PR, 100\_PR\_PerioBox, Chute\_local\_problems are not reported since the  {\sf NSGS-$\star$} are not able to meet the required tolerance for a sufficient percentage of the problems mainly due  to a slow convergence rate. We can see that this problem also appears in the Cubes\_H8 and the BoxesStack test sets.
...
\begin{enumerate}
\item When the timeout value is sufficiently large, we observe the {\sf NSGS-$\star$} are quite robust solvers. In other words, the convergence is slow but robust. Indeed, we are able to find to local solver for each test sets that is able to give a solution at the required accuracy. One of the problem is the fact that there is no universal efficient local solver and the global efficiency depends on the local solver in various ways depending on the test sets.
...
\item  The solver  {\sf NSGS-FP-DS-One} is most efficient on the test sets Bridge\_PR II, KaplasTower II, Chain and BoxesStack. These tests sets seem to be among the easiest one the solve. Nevertheless, the solver seems slow or suffers from robustness issues for other test sets.
\item On the flexible test sets and 945\_SP\_Box\_PL, the best solver is {\sf NSGS-FP-VI-UPK} for a relatively low required tolerance ($\sf tol_{local} =10^{-06}$). For these test sets, an approximate solution of the single contact problems seems sufficient to ensure a efficient convergence towards the solution without entailing robustness.
\item On the test sets 100\_PR\_PerioBox II, KaplasTower,  Chain, Capsules, the solver {\sf NSGS-NSN-$\star$} are the best solvers and behave very well on Bridge\_PR II. It seems that when a tight accuracy is required, the use of  {\sf NSGS-NSN-$\star$} are useful.
\item For the Chute\_1000, Chute\_4000 test sets, we observe large difference between the local formulation of the nonsmooth equations for the Newton solvers. The solver $\sf NSGS-NSN-JM$ is the best solver and really better than $\sf NSGS-NSN-AC$ although their theoretical formulation are very close. Theses two test sets are characterized by difficult local problems where the matrix $W$ is asymmetric.
...
% \subfloat[\scriptsize 100\_PR\_PerioBox]
%    {\includegraphics[width=\figwidth]{../figure/NSGS/LocalSolver/1.0e-08/100/time/profile-LMGC_100_PR_PerioBox.pdf}}
\subfloat[\scriptsize KaplasTower II]
...
\paragraph{Influence of the tolerance of the local solver $tol_{local}$ in {\sf NSGS-FP-VI-UPK} algorithms}
In this section, the tolerance of the local solver is varied and its effect on the global convergence of the solver is reported. In Figure~\ref{fig:NSGS/LocalTol/VI},  we report the performance profile of {\sf NSGS-FP-VI-UPK} algorithms for the $\sf tol_{local}$ in the range $[10^{-04}, 10^{-16}]$. We also report the efficiency of the adaptive value of the local tolerance. The main observations are
\begin{enumerate}
\item For the test sets that are solved a a quicker way, such as BoxesStack,  Capsules, Chain, Chute\_local\_problems, KaplasTower, KaplasTower II,  a tight tolerance on the local solver $10^{-16}$ improves the efficiency and the global convergence of the {\sf NSGS-FP-VI-UPK} solver. It seems that these problems correspond to the problem that are solved quite quickly (see Table~\ref{Tab:fclib-simulation})
\item For the other problems that are longer to solve, that is when we expect more iterations of the  {\sf NSGS-FP-VI-UPK} solver, the adaptive rule or low local tolerances are better.
\end{enumerate}
From these results, it is quite difficult to guess in advance the internal dynamics of the solver. Note that the range of $\tau$ that we used in the graph is quite small so the difference in performance between the solvers are not crucial.
...
% \subfloat[\scriptsize 100\_PR\_PerioBox]
%    {\includegraphics[width=\figwidth]{../figure/NSGS/LocalTol/VI/1.0e-08/100/time/profile-LMGC_100_PR_PerioBox.pdf}}
\subfloat[\scriptsize KaplasTower II]
...
\paragraph{Influence of the tolerance of the local solver $tol_{local}$ in {\sf NSGS-NSN-AC-GP} algorithms}
In Figure~\ref{fig:NSGS/LocalTol},  we report the performance profile of {\sf NSGS-NSN-AC-GP} algorithms for the $\sf tol_{local}$ in the range $[10^{-04}, 10^{-16}]$. We also report the efficiency of the adaptive value of the local tolerance. Except for the test set Chute\_local\_problems,  the main observation is that the local tolerance does not change fundamentally the convergence of the solver. For the test set Chute\_local\_problems, the is no internal dynamics of convergence of the loop on the contact since we have only one contact. It is therefore normal to see that adaptive performs better that the other.
...
\subfloat[\scriptsize 100\_PR\_PerioBox]
   {\includegraphics[width=\figwidth]{../figure/NSGS/LocalTol/1.0e-08/100/time/profile-LMGC_100_PR_PerioBox.pdf}} \\
\subfloat[\scriptsize KaplasTower II]
...
\begin{enumerate}
\item For the test sets 945\_SP\_Box\_PL, 100\_PR\_PerioBox, KaplasTower II, KaplasTower, Chute\_local\_problems, Chute\_local\_problems II, Chute\_4000, Chute\_1000, Capsules, a fixed value of $\rho_\n=\rho_\t=1$ has a dramatic effect on the convergence of the algorithm. The scaling of $\rho$ is of utmost importance for the efficiency and the robustness of the solver. Note that the rule  \eqref{eq:rho-4} that takes into account the condition number of the local matrix $w$ deteriorates the performance for $Chute\_local\_problems, Chute\_local\_problems II, Chute\_4000, Chute\_1000$. In this problem, the local matrix is asymmetric due to large Coriolis effects in the simulation.
\item For the other tests, the choice of $\rho_\n,\rho_\t$ does not really change the results, mainly due the fact that the order of magnitude of the chosen $\rho$ with the rules \eqref{eq:rho-2}, \eqref{eq:rho-3} or \eqref{eq:rho-4} is around $1$.
\end{enumerate}
One of the conclusion of this study is as follows, the rules \eqref{eq:rho-2}, \eqref{eq:rho-3} improves a lot some simulations without increasing the computational tie for the others. Therefore, we advise to use it.
\begin{figure}
  \centering\vspace{-0.5cm}
    \subfiglayout
  \subfloat[\scriptsize LowWall\_FEM]
...
\subfloat[\scriptsize 100\_PR\_PerioBox]
   {\includegraphics[width=\figwidth]{../figure/NSGS/rho/1.0e-08/100/time/profile-LMGC_100_PR_PerioBox.pdf}} \\
\subfloat[\scriptsize KaplasTower II]
   {\includegraphics[width=\figwidth]{../figure/NSGS/rho/1.0e-04/400/time/profile-KaplasTower.pdf}}
\subfloat[\scriptsize KaplasTower]
...
\subfloat[\scriptsize Capsules]
   {\includegraphics[width=\figwidth]{../figure/NSGS/rho/1.0e-08/100/time/profile-Capsules.pdf}}
\subfloat[\scriptsize BoxesStack1]
...
In this section, we study the influence of the contact order within the loop of the {\sf NSGS-AC-QP} solver. We reproduce in Figure~\ref{fig:NSGS/Shuffled} the result of the solvers with the original contact list of the problem {\sf NSGS-AC-GP} with two other ways of iterating over the contacts. The solver {\sf NSGS-AC-GP Shuffled} to a first randomization of the list of contacts at the beginning of the algorithm. In the solver  {\sf NSGS-AC-GP Fully shuffled}, the is shuffled at each iteration.
\begin{enumerate}
\item The solver  {\sf NSGS-AC-GP Fully shuffled} performs really better on the flexible test sets
\item For the rigid test sets, we reproduce here only the test set  100\_PR\_PerioBox II because the other test sets behave identically. The  {\sf NSGS-AC-GP Fully shuffled} has a really bad influence on the convergence of the solver. It seems that is modify the internal dynamics of the solver in a way the rate of convergence is really decreased.
\end{enumerate}
...
% \subfloat[\scriptsize 100\_PR\_PerioBox]
%    {\includegraphics[width=\figwidth]{../figure/NSGS/Shuffled/1.0e-08/100/time/profile-LMGC_100_PR_PerioBox.pdf}} \\
% \subfloat[\scriptsize KaplasTower II]
%    {\includegraphics[width=\figwidth]{../figure/NSGS/Shuffled/1.0e-04/400/time/profile-KaplasTower.pdf}} 
% \subfloat[\scriptsize KaplasTower]
...
% \subfloat[\scriptsize Capsules]
%    {\includegraphics[width=\figwidth]{../figure/NSGS/Shuffled/1.0e-08/100/time/profile-Capsules.pdf}} 
% \subfloat[\scriptsize BoxesStack1]
%    {\includegraphics[width=\figwidth]{../figure/NSGS/Shuffled/time/profile-BoxesStack1.pdf}} \\
{\includegraphics[height=\legendheight]{../figure/NSGS/Shuffled/time/profile-BoxesStack1_legend.pdf}} 
...
In Figure~\ref{fig:PSOR}, we study the effect of the relaxation parameter $\omega$ ranging from $[0.5,1.8]$ on the performance.  Two conclusions can be drawn
\begin{enumerate}
\item For the flexible tests, the efficiency of the solver is really improved as we decreased the value of $\omega$. Moreover, this is done without destroyig the robustness of the solver.
\item for the rigid tests, the effect of the relaxation is not clear. For values of $\omega$ greater than $1.0$, the efficiency is improved but the robustness deteriorates and we observe the contrary for the  $\omega$  less than $1.0$
\end{enumerate}
...
\subfloat[\scriptsize 100\_PR\_PerioBox]
   {\includegraphics[width=\figwidth]{../figure/PSOR/1.0e-08/100/time/profile-LMGC_100_PR_PerioBox.pdf}} \\
\subfloat[\scriptsize KaplasTower II]
   {\includegraphics[width=\figwidth]{../figure/PSOR/1.0e-04/400/time/profile-KaplasTower.pdf}} 
\subfloat[\scriptsize KaplasTower]
...
\subfloat[\scriptsize Capsules]
   {\includegraphics[width=\figwidth]{../figure/PSOR/1.0e-08/100/time/profile-Capsules.pdf}} 
\subfloat[\scriptsize BoxesStack1]
...
% \subfloat[\scriptsize 100\_PR\_PerioBox]
%    {\includegraphics[width=\figwidth]{../figure/NSN/1.0e-08/100/time/profile-LMGC_100_PR_PerioBox.pdf}} \\
\subfloat[\scriptsize KaplasTower II]
   {\includegraphics[width=\figwidth]{../figure/NSN/1.0e-04/400/time/profile-KaplasTower.pdf}} 
\subfloat[\scriptsize KaplasTower]
...
\subfloat[\scriptsize Capsules]
   {\includegraphics[width=\figwidth]{../figure/NSN/1.0e-08/100/time/profile-Capsules.pdf}} 
\subfloat[\scriptsize BoxesStack1]
...
\subfloat[\scriptsize 100\_PR\_PerioBox]
   {\includegraphics[width=\figwidth]{../figure/PROX/InternalSolvers/1.0e-08/100/time/profile-LMGC_100_PR_PerioBox.pdf}} \\
\subfloat[\scriptsize KaplasTower II]
   {\includegraphics[width=\figwidth]{../figure/PROX/InternalSolvers/1.0e-04/400/time/profile-KaplasTower.pdf}} 
\subfloat[\scriptsize KaplasTower]
...
\subfloat[\scriptsize Capsules]
   {\includegraphics[width=\figwidth]{../figure/PROX/InternalSolvers/1.0e-08/100/time/profile-Capsules.pdf}} 
\subfloat[\scriptsize BoxesStack1]
...
\subfloat[\scriptsize 100\_PR\_PerioBox]
   {\includegraphics[width=\figwidth]{../figure/PROX/Parameters/nu05/1.0e-08/100/time/profile-LMGC_100_PR_PerioBox.pdf}} \\
\subfloat[\scriptsize KaplasTower II]
   {\includegraphics[width=\figwidth]{../figure/PROX/Parameters/nu05/1.0e-04/400/time/profile-KaplasTower.pdf}}
\subfloat[\scriptsize KaplasTower]
...
\subfloat[\scriptsize Capsules]
   {\includegraphics[width=\figwidth]{../figure/PROX/Parameters/nu05/1.0e-08/100/time/profile-Capsules.pdf}}
\subfloat[\scriptsize BoxesStack1]
...
\subfloat[\scriptsize 100\_PR\_PerioBox]
   {\includegraphics[width=\figwidth]{../figure/PROX/Parameters/nu10/1.0e-08/100/time/profile-LMGC_100_PR_PerioBox.pdf}} \\
\subfloat[\scriptsize KaplasTower II]
   {\includegraphics[width=\figwidth]{../figure/PROX/Parameters/nu10/1.0e-04/400/time/profile-KaplasTower.pdf}}
\subfloat[\scriptsize KaplasTower]
...
\subfloat[\scriptsize Capsules]
   {\includegraphics[width=\figwidth]{../figure/PROX/Parameters/nu10/1.0e-08/100/time/profile-Capsules.pdf}}
\subfloat[\scriptsize BoxesStack1]
...
\subfloat[\scriptsize 100\_PR\_PerioBox]
   {\includegraphics[width=\figwidth]{../figure/PROX/Parameters/nu20/1.0e-08/100/time/profile-LMGC_100_PR_PerioBox.pdf}} \\
\subfloat[\scriptsize KaplasTower II]
   {\includegraphics[width=\figwidth]{../figure/PROX/Parameters/nu20/1.0e-04/400/time/profile-KaplasTower.pdf}}
\subfloat[\scriptsize KaplasTower]
...
\subfloat[\scriptsize Capsules]
   {\includegraphics[width=\figwidth]{../figure/PROX/Parameters/nu20/1.0e-08/100/time/profile-Capsules.pdf}}
\subfloat[\scriptsize BoxesStack1]
...
\subfloat[\scriptsize 100\_PR\_PerioBox]
   {\includegraphics[width=\figwidth]{../figure/OPTI/1.0e-08/100/time/profile-LMGC_100_PR_PerioBox.pdf}} \\
\subfloat[\scriptsize KaplasTower II]
   {\includegraphics[width=\figwidth]{../figure/OPTI/1.0e-04/400/time/profile-KaplasTower.pdf}}
\subfloat[\scriptsize KaplasTower]
...
\subfloat[\scriptsize Capsules]
   {\includegraphics[width=\figwidth]{../figure/OPTI/1.0e-08/100/time/profile-Capsules.pdf}}
\subfloat[\scriptsize BoxesStack1]
...
\subfloat[\scriptsize 100\_PR\_PerioBox]
   {\includegraphics[width=\figwidth]{../figure/COMP/zoom/1.0e-08/100/time/profile-LMGC_100_PR_PerioBox.pdf}} \\
\subfloat[\scriptsize KaplasTower II]
   {\includegraphics[width=\figwidth]{../figure/COMP/zoom/1.0e-04/400/time/profile-KaplasTower.pdf}}
\subfloat[\scriptsize KaplasTower]
...
\subfloat[\scriptsize Capsules]
   {\includegraphics[width=\figwidth]{../figure/COMP/zoom/1.0e-08/100/time/profile-Capsules.pdf}}
\subfloat[\scriptsize BoxesStack1]
...
\item {\sf NSGS-$\star$}
  ...
  \item robust
  \item slow
  \item local solvers must preserve robutness.
  \end{enumerate}
\item {\sf NSN-$\star$}
  \begin{enumerate}
  \item Choice of rho and robustness convergence. more studies
  \end{enumerate}
\end{itemize}
