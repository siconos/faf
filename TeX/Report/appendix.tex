...
\begin{definition}[\citet{Rockafellar.Wets1997}]\label{def:MM}
...
\subsection*{Sub-differential of the Euclidean norm.}

The sub-differential of the Euclidean norm in $\RR^n$ is given by:
\begin{equation}
  \label{eq:sub-norm}
  \partial \|z\| =
  \left\{
...
\subsection*{Euclidean projection on the unit ball.}
Let $B=\{x \in \RR^n, \|x\| \leq 1\}$. The Euclidean projection on the unit ball is given by: 
\begin{equation}
  \label{eq:projD}
  P_{B}(z) =\left\{
  \begin{array}{lcl}
...
\begin{equation}
  ...
  \partial P_{B}(z) =\left\{
  \begin{array}{lcl}
   I &\text{if }& z \in B \setminus \partial B \\
   I + (s-1)z z^\top , s\in [0,1]   &\text{if }& z \in  \partial B \\
...
Let $K=\{x = [x_\n x_\t]^T \in \RR^3, x_\n \in \RR, \|x_{\t}\| \leq \mu x_{\n}\}$ be the second order cone in $\RR^3$. The Euclidean projection on $K$ is
\begin{equation}
  \label{eq:proj}
  P_{K}(z) =\left\{
  \begin{array}{lcl}
...
\end{equation}
where $ \Phi_{x y}(u,r) \in \partial_{x}[F_{\vitwo}^\nat]_{y}(u,r)$. Since $\Phi_{u u}(u,r) = I $, a reduction of the system is performed in practice and Algorithm~\ref{Algo:NSN} is applied or $z =r$ with
\begin{equation}
  \label{eq:phiphi_ter}
  \begin{cases}
    G(z) = [F_{\vitwo}^\nat]_{r}(Wr+q,r) \\
...
If the variant~\eqref{eq:Moreau--Jean-1} is chosen, the computation of $\Phi_{r_\t \bullet}$  simplifies in
\begin{equation}
