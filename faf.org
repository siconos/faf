...
+ Implement Fixed point Iteration
  + Which internal solver ? 
    + the internal solver must use the convexity of the problem
    + Block-splitting + local solver (PSOR + list of solver) Copy of nsgs with psor options
      
      
      
Newton method of convex SOCCP
