...
** TODO List of actions 
  + Formulation of hybrid Solvers 
  + MB : Rerun computation  
  + VA : Clean Redaction Note from the paper Why this stuff and not this one
  + VA : investigate the best journal to submit (and format)
  + OH : Fclib. Thinking of howto bind with Matlab 
  + OH : fixes + new parts in paper
  + MB : fclib and swig python
